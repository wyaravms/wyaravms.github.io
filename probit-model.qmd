...
Dado o MLG Probito para variáveis binárias,
\begin{equation*}
\begin{array}{rclll}
...
Em que para tornar o MCMC para este modelo mais eficiente, temos o modelo 'aumentado' com a inserção de uma variável auxiliar $u$, dado por:
\begin{equation*}
\begin{array}{rclll}
...
Iremos demonstrar a equivalência dos modelos com a variável auxiliar $u_{i}$ e do modelo sem a variável auxiliar $u_{i}$. Observe que, a probabilidade marginal do modelo com a variável auxiliar, é dada por:
\begin{equation*}
\begin{array}{rclll}
...
Na segunda parte desta questão, a fim de encontrar as condicionais completas referentes os parâmetros desconhecido, precisamos completar o modelo. Assim, iremos atribuir uma priori normal para os $\beta$'s, ou seja,
\begin{equation*}
\begin{array}{rclll}
...
Portanto, o modelo 'aumentado' terá a seguinte estrutura hierárquica:
\begin{equation*}
\begin{array}{lclll}
Y_{i} & = & 
\begin{cases}
...
Assim, podemos encontrar a distribuição conjunta a posteriori para os parâmetros, a qual é dada por:
\begin{equation*}
\begin{array}{rclll}
...
Começando pela análise do produto das distribuições: $\pi(\textbf{Y}\mid\textbf{u})\pi( \textbf{u}\mid\boldsymbol{\beta})$, temos:
\begin{equation*}
\begin{array}{rclll}
...
Logo, a distribuição conjunta a posteriori para os parâmetros, é dada por:
\begin{equation*}
\begin{array}{rclll}
...
Portanto, as distribuições condicionais completas para $\beta$ e $u_{i}$, para $i = \{1, \cdots, n\}$ são dadas por:
\begin{equation*}
\begin{array}{rclll}
